 ...
 */
export async function fetchBungieProfile(includeCharacters = false): Promise<BungieUserResponse> {
  ...
  return includeCharacters
    ? getBungieUserWithCharacters(token)
    : getBungieUser(token)
}
 ...
 */
export function useBungieServerProfile(options: {
  includeCharacters?: boolean
  enabled?: boolean
  queryOptions?: Omit<UseQueryOptions<BungieUserResponse, Error>, 'queryKey' | 'queryFn' | 'enabled'>
} = {}) {
  const {
